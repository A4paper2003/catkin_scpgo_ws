...
void gpsHandler(const sensor_msgs::NavSatFix::ConstPtr &_gps)
{
    if(useGPS) {
        ...
        if(gps_count++ % 10 == 0) {  // Print every 10 messages
            ROS_INFO("GPS received: lat=%.8f, lon=%.8f, alt=%.3f, time=%.3f",
                    _gps->latitude, 
                    _gps->longitude, 
                    _gps->altitude,
                    _gps->header.stamp.toSec());
        }   
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
    ...
    // Add debug output 
    if(count++ % 100 == 0) { 
        // Print every 100 messages to avoid spam 
        ROS_INFO("IMU received: orientation(w=%.3f, x=%.3f, y=%.3f, z=%.3f)", 
            curr_IMU.orientation. w, 
...
void initNoises( void )
{
    ...
    double bigNoiseTolerentToXY = 1000000.0; // 1e9
    double gpsAltitudeNoiseScore = 1000000.0; // if height is misaligned after loop clsosing, use this value bigger
		...
		while ( !odometryBuf.empty() && !fullResBuf.empty() && get_imu) 
        {
            ...
            while (!gpsBuf.empty()) {
                ...
                if( abs(thisGPSTime - timeLaserOdometry) < eps ) {
                    ...
                    //Debug output
                    ROS_INFO("GPS matched for keyframe: lat=%.8f, lon=%.8f, alt=%.3f, time_diff=%.4f", 
                        currGPS->latitude, currGPS->longitude, currGPS->altitude,
                        abs(thisGPSTime - timeLaserOdometry)); 

                    break;
                } else {
                    hasGPSforThisKF = false;
                    // ADD DEBUG FOR MISSED GPS
                    ROS_DEBUG("GPS time mismatch: GPS_time=%.3f, Odom_time=%.3f, diff=%.4f",
                        thisGPSTime, timeLaserOdometry, abs(thisGPSTime - timeLaserOdometry));
                }
                gpsBuf.pop();
            }

            // ADD DEBUG FOR NO GPS
            if(!hasGPSforThisKF) {
                ROS_WARN_THROTTLE(2.0, "No GPS data matched for this keyframe");
            }
            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    if(hasGPSforThisKF) {
                        ...
                        mtxRecentPose.unlock();
                        if(currGPS->position_covariance[0]<= 0.5 && currGPS->position_covariance[4]<= 0.5)
                        {
